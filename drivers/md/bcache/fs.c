				    ...
				    loff_t offset, loff_t len)
{
	...
	if (mode & FALLOC_FL_ZERO_RANGE) {
		...
		block_start	= round_up(offset, PAGE_CACHE_SIZE);
		block_end	= round_down(offset + len, PAGE_CACHE_SIZE);
	} else {
		block_start	= round_down(offset, PAGE_CACHE_SIZE);
		block_end	= round_up(offset + len, PAGE_CACHE_SIZE);
	}
...
do_io:
	if (check_make_i_size_dirty(ei, page_offset(page) + PAGE_CACHE_SIZE)) {
